...
class UpdateSettings(StatesGroup): # класс создается как контейнер. он нужен для логического объединения функций, относящихся к регистрации
...
async def profile(message: Message):
    # 1. Создаем объект конструктора клавиатуры
    builder = InlineKeyboardBuilder()
    # 2. Добавляем кнопки (текст и callback_data — то, что бот поймает при нажатии)
    ...
    # 3. Указываем, как распределить кнопки (например, по 1 в ряд)
    builder.adjust(1)
    # 4. Отправляем сообщение вместе с клавиатурой
    await message.answer(
        "Выбери, что тебя интересует 👇",
        reply_markup=builder.as_markup()  # Превращаем билдер в готовую разметку
    )
...
@router.message(UpdateSettings.waiting_for_data)# Ловим сообщение ТОЛЬКО если пользователь находится в этом состоянии.
...
async def process_user_info(callback: CallbackQuery, state: FSMContext):  # <- Просим aiogram дать нам state
    # Гасим анимацию загрузки на кнопке
    await callback.answer()
    ...
    await state.set_state(UpdateSettings.waiting_for_data)# Метод set_state() принимает только один основной параметр — это объект состояния, который мы создали (например, Registration.waiting_for_data).
...
# Этот хэндлер поймает сообщение, только если у пользователя НЕТ активного состояния
@router.message()
async def echo_message(message: Message):
    await message.answer("Я тебя не понимаю. Напиши /menu.")
