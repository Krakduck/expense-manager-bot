...
async def help_user(message: Message):
    help_text = (
        "🤖 **Многофункциональный Бот-Помощник**\n\n"

        "💰 **РАСХОДЫ И ДОЛГИ**\n"
        "🔹 /spent — Добавить новую трату (например: `1000 пицца все` или `25 вода @username`).\n"
        "🔹 /debts — Показать баланс чата и оптимальную цепочку переводов для закрытия долгов.\n"
        "🔹 /clear_debt — Сбросить всю историю трат группы (доступно только админам).\n\n"

        "📅 **ОТ СЛЕЖИВАНИЕ СОБЫТИЙ И СЕРИЙ**\n"
        "🔹 /event — Меню управления событиями. Позволяет создавать и удалять трекеры, а также продлевать серии дней вручную или автоматически.\n\n"

        "💬 **ЦИТАТНИК**\n"
        "🔹 /save — Сохранить сообщение в базу цитат (вызывать *в ответ* на нужное сообщение).\n"
        "🔹 /citations — Посмотреть все сохраненные цитаты или найти фразы конкретного человека.\n\n"

        "⚙️ **ПРОЧЕЕ И НАСТРОЙКИ**\n"
        "🔹 /profile — Настройки твоего профиля (изменение имени, установка автоответа на пинг).\n"
        "🔹 /random — Выбрать случайного участника беседы.\n"
        "🔹 /all — Тегнуть всех зарегистрированных участников чата.\n"
        "🔹 /del_profile — Удалить свой аккаунт или очистить базу (для админов)."
    )
